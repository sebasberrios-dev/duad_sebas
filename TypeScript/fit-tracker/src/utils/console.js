...
export function printUserInfo(user) {
    ...
    console.log("══════════════════════════════════");
    console.log(`👤 Usuario: ${name}, Edad: ${age}, Peso: ${bodyWeight} kg, Nivel: ${level}`);
    console.log("──────────────────────────────────");
    console.log(`💎Membresía:`);
    console.log(`Plan: ${plan}, Fecha de inicio: ${formatDate(startDate)}, Estado: ${status === "active" ? "✅ Activa" : "❌ Inactiva"}`);
    console.log("══════════════════════════════════\n");
}
export function printRoutine(routine, user) {
    ...
    printUserInfo(user);
    console.log(`📋 Rutina: ${routineName} | Inicio: ${formatDate(routineStartDate)}`);
    console.log(" ────────────────────────────────────");
    for (const workout of workouts) {
        ...
        for (const exercise of workout.exercises) {
            ...
            if (category === "Cardio") {
                const exerciseCalories = routineCalories.perExercise.find((e) => e.exerciseName === exerciseName)?.calories ?? 0;
                const pace = calculatePace(durationMinutes, details.distanceKm);
                console.log(`${status === "completed" ? "✅" : status === "skipped" ? "❌" : "⏳"}  ${exerciseName} [${category}], ${duration}, ${pace}min/km | ${exerciseCalories} kcal quemadas`);
            }
            if (category === "Fuerza") {
                console.log(`${status === "completed" ? "✅" : status === "skipped" ? "❌" : "⏳"}  ${exerciseName} [${category}], ${duration}`);
                for (const [index, set] of details.sets.entries()) {
                    console.log(` - Serie ${index + 1} | ${set.reps} reps | ${set.weightKg} kg`);
                }
            }
...
export function printWeeklyLoad(routine, user) {
    const { bodyWeight, level } = user;
    const { total, formatTotalDuration, cardioDuration, strengthDuration, flexDuration, } = getTotalDuration(routine);
    const { totalCalories } = calculateAllCalories(routine, bodyWeight, level);
    const { cal, uniqueDays } = calculateWeeklyAvgCalories(routine, totalCalories);
    console.log("📊 Carga semanal");
    console.log(`${formatTotalDuration} | ${totalCalories} kcal | Promedio: ${cal} kcal (${uniqueDays} días entrenados)`);
    console.log(`🏃 Cardio: ${cardioDuration}   💪 Fuerza: ${strengthDuration}  🧘 Flexibilidad: ${flexDuration}`);
    if (uniqueDays > 5 || total > 300) {
        console.log("⚠️ Considera tener un día más de descanso o horas de entrenamiento esta semana.");
    }
...
export function printCatalogReport(report, filter) {
    ...
    console.log("");
    console.log(`Catálogo local: ${report.totals.local} ejercicios | Desde API: ${report.totals.api} ejercicios`);
    console.log("══════════════════════════════════\n");
}
