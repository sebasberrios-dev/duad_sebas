...
def get_products():
    try:
        ...
        return jsonify({'products': [dict(p) for p in products]}), 200
...
def create_product():
    try:
        ...
        if not name or not description or not category or price is None or stock is None:
...
def update_product(product_id):
    try:
        ...
        if products_repo.update(product_id, update_data):
            ...
            return jsonify({'message': 'product updated', 'product': dict(updated_product)}), 200
...
@products_bp.route('/products/reduce_stock/<int:product_id>', methods=['POST'])
@require_auth
