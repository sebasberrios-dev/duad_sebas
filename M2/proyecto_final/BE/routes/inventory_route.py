...
def get_my_inventory():
    # Obtiene el inventario completo de un personaje con todos sus items en un juego específico
    try:
        character_id = request.args.get("character_id", type=int)
        game_id = request.args.get("game_id", type=int)
        
        if not character_id or not game_id:
            return jsonify({"error": "character_id and game_id are required"}), 400
        
        # Obtener inventario con todos los items
        inventory_items = inventory_repo.get_inventory_with_items(character_id, game_id)
...
# Crea un inventario para un personaje en una partida
@inventory_route.route("/inventory/new", methods=["POST"])
@require_auth
def create_inventory():
    # Crea un inventario vacío para un personaje en un juego específico
    try:
        ...
        # Verificar si ya tiene inventario en este juego
        existing = inventory_repo.read_by_character_and_game(character_id, game_id)
        if existing:
            return jsonify({"error": "Character already has an inventory in this game"}), 409

        result = inventory_repo.create(character_id, game_id)
...
# Elimina un inventario (solo admins)
...
# Actualiza un item del catálogo (solo DMs)
...
# Elimina un item del catálogo (solo admins)
...
def add_item_to_inventory():
    try:
        ...
        if not character_id or not game_id or not item_id:
            return jsonify({"error": "character_id, game_id and item_id are required"}), 400

        item = item_repo.read_by_id(item_id)
        if not item:
            return jsonify({"error": "Item not found"}), 404

        inventory = inventory_repo.read_by_character_and_game(character_id, game_id)
        if not inventory:
            result = inventory_repo.create(character_id, game_id)
...
# Remueve un item del inventario de un personaje
...
def update_item_quantity():
    try:
        ...
        if not character_id or not game_id or not item_id or new_quantity is None:
            return jsonify({"error": "character_id, game_id, item_id and quantity are required"}), 400
